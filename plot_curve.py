...
def plot_beauty():
    ...
    for train_idx, test_idx in cv.split(X, y):
        ...
        acc, _, _, _, _, y_score = GEDFN.gedfn(X_train, X_test,
                                               to_categorical(y_train), to_categorical(y_test), right, right)
        fpr, tpr, _ = roc_curve(y_test, y_score)
        ...
        ################DFN################################
        acc, _, _, _, _, y_score = dfn(X_train, X_test,
    ...
    axes[1].plot(recall, precision, label='GEMLP:AP=' + str(ap), color='purple')
    ################DFN################################

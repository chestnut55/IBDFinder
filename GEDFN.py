...
with tf.Session() as sess:
    ...
    for epoch in range(training_epochs):
        ...
        if epoch % display_step == 0:
            ...
            acc, y_s, _= sess.run([accuracy, y_score, layer1_weights], feed_dict={x: x_train, y: y_train, keep_prob: 1})
    ...
    ## Testing cycle
    acc, y_s, l1_weights = sess.run([accuracy, y_score, layer1_weights], feed_dict={x: x_test, y: y_test, keep_prob: 1})

    y_pred = np.argmax(np.where(y_s > 0.5, 1, 0), axis=1)
